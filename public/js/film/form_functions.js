...
{
    console.log($('#search-'+title+'-dropdown').empty());
...
$(document).ready(function() {
    // CSRF used for all POST/PUT/DELETE
    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
    var film_id = $('meta[name="film-id-meta"]').attr('content');
    const film = new Film(film_id);
    film.render();
    film.render_edit();
    $('#film-edit-icon').on('click', function() {
        $('#film-info').toggle('display');
        $('#edit-film-info').children('div').toggle('display');
    });
});